...
def setup_mlx_environment():
    ...
    if not os.getenv("MLX_CACHE_DIR"):
        os.environ["MLX_CACHE_DIR"] = str(Path.home() / ".cache" / "mlx-models")
...
def test_check_mlx_installation():
    ...
    try:
        ...
        try:
            import mlx
            import mlx.core
            logger.info(f"MLX版本: {mlx.__version__ if hasattr(mlx, '__version__') else '未知'}")
            
            # 检查是否支持MPS (Metal Performance Shaders)
            if hasattr(mlx.core, "metal_is_available"):
                metal_available = mlx.core.metal_is_available()
                logger.info(f"MPS可用: {'是' if metal_available else '否'}")
            else:
                logger.info("无法检测MPS可用性")
            
            return True
        except ImportError as e:
            logger.error(f"MLX未安装或无法导入: {e}")
            return False
    except Exception as e:
        logger.error(f"检查MLX安装时出错: {e}")
        traceback.print_exc()
        return False
...
def main():
    ...
    try:
        ...
        # 测试2: 检查MLX安装情况
        print("\n2. 测试MLX安装情况:")
        if test_check_mlx_installation():
            print("✅ 测试通过: MLX已正确安装")
        else:
            print("❌ 测试失败: MLX未正确安装")
            return
